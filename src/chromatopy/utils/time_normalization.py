...
def discrete_time_shift(refy, lower, upper, time_column, signal_column):
    """
    ...
    """
    refy = refy.loc[(refy[time_column] < upper) & (refy[time_column] > lower)]
    refy = refy.reset_index(drop=True)
    if signal_column not in refy.columns or refy.empty:
        return None
    signal = np.nan_to_num(refy[signal_column].to_numpy(dtype=float), nan=0.0)
    prominence = max(np.nanmax(signal) * 0.05, 1e-9)
    pks, _ = find_peaks(signal, prominence=prominence)
    if pks.size == 0:
        return None
    best_idx = pks[np.argmax(signal[pks])]
    return float(refy.iloc[best_idx][time_column])
...
def time_normalization(
    ...
    *,
    ...
    """
    if not enabled or not reference_trace:
        for d in data:
            d["rt_corr"] = d[time_column]
        return {"data": data, "iref": True}

    if reference_window is None:
        reference_window = [float(np.nanmin(data[0][time_column])), float(np.nanmax(data[0][time_column]))]

    ref_time = discrete_time_shift(
        data[0],
        lower=reference_window[0],
        upper=reference_window[1],
        time_column=time_column,
        signal_column=reference_trace,
    )
    if ref_time is None:
        for d in data:
            d["rt_corr"] = d[time_column]
    ...
    for d in data:
        time_change = discrete_time_shift(
            d,
            lower=reference_window[0],
            upper=reference_window[1],
            time_column=time_column,
        ...
        if time_change is None:
            ...
            d["rt_corr"] = d[time_column] - time_change + ref_time
    iref = True
    return {
        "data": data,
        "iref": iref}
